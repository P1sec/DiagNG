...
class ExtLogConfigRsp(ReadWriteKaitaiStruct):
    ...
    def _read(self):
        ...
        if _on == diag_log_config_f_req.DiagLogConfigFReq.Operation.disable_op:
            ...
            self.payload = ExtLogConfigRsp.Disable(
            ...
            self._raw_payload = self._io.read_bytes_full()
            _io__raw_payload = KaitaiStream(BytesIO(self._raw_payload))
            self.payload = ExtLogConfigRsp.GetMask(
                _io__raw_payload, self, self._root
            )
            self.payload._read()
        elif (
            ...
            self._raw_payload = self._io.read_bytes_full()
            _io__raw_payload = KaitaiStream(BytesIO(self._raw_payload))
            self.payload = ExtLogConfigRsp.RetrieveIdRanges(
                _io__raw_payload, self, self._root
            )
            self.payload._read()
        elif (
            ...
            self._raw_payload = self._io.read_bytes_full()
            _io__raw_payload = KaitaiStream(BytesIO(self._raw_payload))
            self.payload = ExtLogConfigRsp.SetMask(
                _io__raw_payload, self, self._root
            )
            self.payload._read()
        else:

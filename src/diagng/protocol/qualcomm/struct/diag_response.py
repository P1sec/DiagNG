...
class DiagResponse(ReadWriteKaitaiStruct):
    ...
    def _read(self):
        ...
        _on = self.cmd_code
        if _on == diag_cmd_code.DiagCmdCode.DiagCmd.cmd_ext_f:
            pass
            self._raw_payload = self._io.read_bytes_full()
            _io__raw_payload = KaitaiStream(BytesIO(self._raw_payload))
            self.payload = diag_cmd_ext_f.DiagCmdExtF(_io__raw_payload)
            self.payload._read()
        elif _on == diag_cmd_code.DiagCmdCode.DiagCmd.log_config_f:
            pass
            self._raw_payload = self._io.read_bytes_full()
            _io__raw_payload = KaitaiStream(BytesIO(self._raw_payload))

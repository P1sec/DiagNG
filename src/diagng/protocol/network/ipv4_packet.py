...
class Ipv4Packet(ReadWriteKaitaiStruct):
    ...
    def _read(self):
        ...
        self.options = Ipv4Packet.Ipv4Options(
            _io__raw_options, self, self._root
        )
        self.options._read()
    ...
    def _write__seq(self, io=None):
        ...
        _io__raw_options = KaitaiStream(
            BytesIO(bytearray(self.ihl_bytes - 20))
        )
        ...
        def handler(parent, _io__raw_options=_io__raw_options):
            ...
            if len(self._raw_options) != self.ihl_bytes - 20:
                raise kaitaistruct.ConsistencyError(
                    'raw(options)', self.ihl_bytes - 20, len(self._raw_options)
                )
            parent.write_bytes(self._raw_options)

        _io__raw_options.write_back_handler = KaitaiStream.WriteBackHandler(
            _pos2, handler
        )
        self.options._write__seq(_io__raw_options)
        _io__raw_body = KaitaiStream(
            BytesIO(bytearray(self.total_length - self.ihl_bytes))
        )
    ...
    def _check(self):
        if len(self.src_ip_addr) != 4:
            raise kaitaistruct.ConsistencyError(
                'src_ip_addr', 4, len(self.src_ip_addr)
            )
        if len(self.dst_ip_addr) != 4:
            raise kaitaistruct.ConsistencyError(
                'dst_ip_addr', 4, len(self.dst_ip_addr)
            )
        if self.options._root != self._root:
            raise kaitaistruct.ConsistencyError(
                'options', self._root, self.options._root
            )
        if self.options._parent != self:
            raise kaitaistruct.ConsistencyError(
                'options', self, self.options._parent
            )
        if self.body.protocol_num != self.protocol:
            raise kaitaistruct.ConsistencyError(
                'body', self.protocol, self.body.protocol_num
            )
        self._dirty = False
    ...
    class Ipv4Option(ReadWriteKaitaiStruct):
        ...
        def _read(self):
            ...
            self.body = self._io.read_bytes(
                (self.len - 2 if self.len > 2 else 0)
            )
            self._dirty = False
    ...
    class Ipv4Options(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            ...
            for i in range(len(self.entries)):
                ...
                if self._io.is_eof():
                    raise kaitaistruct.ConsistencyError(
                        'entries', 0, self._io.size() - self._io.pos()
                    )
        ...
        def _check(self):
            for i in range(len(self.entries)):
                ...
                if self.entries[i]._root != self._root:
                    raise kaitaistruct.ConsistencyError(
                        'entries', self._root, self.entries[i]._root
                    )
                if self.entries[i]._parent != self:
                    raise kaitaistruct.ConsistencyError(
                        'entries', self, self.entries[i]._parent
                    )

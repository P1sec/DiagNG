...
class Pcap(ReadWriteKaitaiStruct):
    ...
    def _read(self):
        self.magic_number = KaitaiStream.resolve_enum(
            Pcap.Magic, self._io.read_u4be()
        )
        ...
        for i in range(len(self.packets)):
            ...
            if self._io.is_eof():
                raise kaitaistruct.ConsistencyError(
                    'packets', 0, self._io.size() - self._io.pos()
                )
    ...
    def _check(self):
        if self.hdr._root != self._root:
            raise kaitaistruct.ConsistencyError(
                'hdr', self._root, self.hdr._root
            )
        if self.hdr._parent != self:
            raise kaitaistruct.ConsistencyError('hdr', self, self.hdr._parent)
        for i in range(len(self.packets)):
            pass
            if self.packets[i]._root != self._root:
                raise kaitaistruct.ConsistencyError(
                    'packets', self._root, self.packets[i]._root
                )
            if self.packets[i]._parent != self:
                raise kaitaistruct.ConsistencyError(
                    'packets', self, self.packets[i]._parent
                )
    ...
    class Header(ReadWriteKaitaiStruct):
        ...
        def _read_le(self):
            ...
            if not self.version_major == 2:
                raise kaitaistruct.ValidationNotEqualError(
                    2, self.version_major, self._io, '/types/header/seq/0'
                )
            ...
            self.network = KaitaiStream.resolve_enum(
                Pcap.Linktype, self._io.read_u4le()
            )
            self._dirty = False
        ...
        def _read_be(self):
            ...
            self.network = KaitaiStream.resolve_enum(
                Pcap.Linktype, self._io.read_u4be()
            )
            self._dirty = False
        ...
        def _check(self):
            if not self.version_major == 2:
                raise kaitaistruct.ValidationNotEqualError(
                    2, self.version_major, None, '/types/header/seq/0'
                )
            self._dirty = False

...
class ProtocolBody(ReadWriteKaitaiStruct):
        ...
        if _on == ProtocolBody.ProtocolEnum.hopopt:
            ...
            if self.body._root != self._root:
                raise kaitaistruct.ConsistencyError(
                    'body', self._root, self.body._root
                )
            if self.body._parent != self:
                raise kaitaistruct.ConsistencyError(
                    'body', self, self.body._parent
                )
        elif _on == ProtocolBody.ProtocolEnum.ipv4:
            pass
        elif _on == ProtocolBody.ProtocolEnum.ipv6_nonxt:
    ...
    class OptionHopByHop(ReadWriteKaitaiStruct):
        ...
        def _read(self):
            ...
            self.body = self._io.read_bytes(
                (self.hdr_ext_len - 1 if self.hdr_ext_len > 0 else 1)
            )
            self.next_header = ProtocolBody(
                self.next_header_type, self._io, self, self._root
            )
        ...
        def _check(self):
            ...
            if self.next_header._root != self._root:
                raise kaitaistruct.ConsistencyError(
                    'next_header', self._root, self.next_header._root
                )
            if self.next_header._parent != self:
                raise kaitaistruct.ConsistencyError(
                    'next_header', self, self.next_header._parent
                )
            if self.next_header.protocol_num != self.next_header_type:
                raise kaitaistruct.ConsistencyError(
                    'next_header',
                    self.next_header_type,
                    self.next_header.protocol_num,
                )
            self._dirty = False
    ...
    def protocol(self):
        ...
        self._m_protocol = KaitaiStream.resolve_enum(
            ProtocolBody.ProtocolEnum, self.protocol_num
        )

...
class EthernetFrame(ReadWriteKaitaiStruct):
    ...
    def _read(self):
        ...
        self.ether_type_1 = KaitaiStream.resolve_enum(
            EthernetFrame.EtherTypeEnum, self._io.read_u2be()
        )
    ...
    def _write__seq(self, io=None):
        ...
        if _on == EthernetFrame.EtherTypeEnum.ipv4:
            ...
            def handler(parent, _io__raw_body=_io__raw_body):
                ...
                if not parent.is_eof():
                    raise kaitaistruct.ConsistencyError(
                        'raw(body)', 0, parent.size() - parent.pos()
                    )
            ...
            if not self._io.is_eof():
                raise kaitaistruct.ConsistencyError(
                    'body', 0, self._io.size() - self._io.pos()
                )

    def _check(self):
        if len(self.dst_mac) != 6:
            raise kaitaistruct.ConsistencyError(
                'dst_mac', 6, len(self.dst_mac)
            )
        if len(self.src_mac) != 6:
            raise kaitaistruct.ConsistencyError(
                'src_mac', 6, len(self.src_mac)
            )
        if self.ether_type_1 == EthernetFrame.EtherTypeEnum.ieee_802_1q_tpid:
            pass
            if self.tci._root != self._root:
                raise kaitaistruct.ConsistencyError(
                    'tci', self._root, self.tci._root
                )
            if self.tci._parent != self:
                raise kaitaistruct.ConsistencyError(
                    'tci', self, self.tci._parent
                )
    ...
    def ether_type(self):
        ...
        self._m_ether_type = (
            self.ether_type_2
            if self.ether_type_1
            == EthernetFrame.EtherTypeEnum.ieee_802_1q_tpid
            else self.ether_type_1
        )
